...
public class GUI extends JFrame implements ActionListener {
	...
	private List<SolutionStep> solutionSteps = new ArrayList<SolutionStep>();
	private int lastSolutionStep = 0;

	GUI(Board board, Solver solver) {
		super("Судоку");
		this.board = board;
		this.solver = solver;
		...
		for (Command c : Command.values()) {
			...
			b.setBounds(BUTTON_X_POSITION, BUTTON_Y_POSITION + button.size() * (BUTTON_HEIGHT + 20), BUTTON_WIDTH,
					BUTTON_HEIGHT);
			button.add(b);
			add(b);
		}
		...
		public void paint(Graphics g) {
			...
			for (int delta = -1; delta <= 1; delta++) {
				for (int i = 0; i <= Board.SIZE; i++) {
					g.drawLine(0, 3 * i * (FIELD_SIZE + 1) + delta, Board.DIMENSION * (FIELD_SIZE + 1),
							3 * i * (FIELD_SIZE + 1) + delta);
				}
			}
			for (int delta = -1; delta <= 1; delta++) {
				for (int i = 0; i <= Board.SIZE; i++) {
					g.drawLine(3 * i * (FIELD_SIZE + 1) + delta, 0, 3 * i * (FIELD_SIZE + 1) + delta,
							Board.DIMENSION * (FIELD_SIZE + 1));
				}
			}
		}

		private final Font font = new Font("Times New Roman", Font.BOLD, FIELD_SIZE / 2);
				...
				for (int i = 0; i < Board.SIZE; i++) {
					for (int j = 0; j < Board.SIZE; j++) {
						if (board.isPossible(row, column, i * Board.SIZE + j)) {
							g.drawString("" + (i * Board.SIZE + j + 1), xPosition + 10 + (2 * j * FIELD_SIZE / 7),
									yPosition + 4 + fm.getHeight() + (2 * i * FIELD_SIZE / 7));
						}
					}
				}
			}
	...
	private class Button extends JButton {
		...
		Button(Command pCommand, ActionListener pListener) {
			super();
			setActionCommand(pCommand.mSymbol);
			setText(pCommand.mSymbol);
			setToolTipText(pCommand.mToolTip);
	...
	private List<Move> load() {
		...
		if (value == JFileChooser.APPROVE_OPTION) {
			return board.load(fc.getSelectedFile());
		}
		return null;
	}
	...
	void _setHint(List<Move> moves) {
		if (moves == null || moves.size() == 0) {
			message.setText("Kraj");
			return;
		}

		String text = "";
		for (Move move : moves) {
			if (!move.operation.isHintOperation)
				continue;

			if (move.operation.backgroundColor != null) {
				board.setColor(move.row, move.column, move.operation.backgroundColor);
			}
